...
Class Home extends MY_Controller
{
	function index()
	{
	    ...
	    $maxView = $this->project_model->get_list($input);
		$this->data['maxView']  = $maxView;
		 //Dự án mới được setup ra trang chủ
		 $this->load->model('sunhouse_model');
		 $input = array();
		 $input['where'] = array('is_home' => 0);
		 $input['order'] = array('id', 'DESC');
	     $input['limit'] = array(9, 0);
		 $isHome = $this->project_model->get_list($input);
		 ...
		 $input['where'] = array('is_home' => 0);
		 $input['order'] = array('id', 'DESC');
		 $maxViewSunHouse = $this->sunhouse_model->get_list($input);
		 $this->data['maxViewSunHouse']  = $maxViewSunHouse;
		  //Dự án nhà phố mới được setup ra trang chủ
		  $this->load->model('sunhouse_model');
		  $input = array();
		  $input['where'] = array('is_home' => 0);
		  $input['order'] = array('id', 'DESC');
		  $input['limit'] = array(8, 0);
		  $isHomeSunHouse = $this->sunhouse_model->get_list($input);
		  $this->data['isHomeSunHouse']  = $isHomeSunHouse;
